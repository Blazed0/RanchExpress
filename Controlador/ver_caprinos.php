...
function filtrado($conn,$sexo){
    $sql = "SELECT animal.imagen_animal, animal.nombre, animal.codigo_animal, usuario.nombre AS nombre_usuario FROM animal LEFT JOIN usuario ON animal.id_usuario = usuario.id_usuario WHERE especie = 'Caprino' AND sexo = ? AND etapa_edad = 'Adulto';";
    ...
    $sqlCrias = "SELECT animal.imagen_animal, animal.nombre, animal.codigo_animal, usuario.nombre AS nombre_usuario FROM animal LEFT JOIN usuario ON animal.id_usuario = usuario.id_usuario WHERE especie = 'Caprino' AND etapa_edad = 'Cria';";
    $resultado = $conn->query($sqlCrias);
    $htmlCrias = '';
    if ($resultado && $resultado->num_rows > 0){
        while ($rows = $resultado->fetch_assoc()){
            $htmlCrias = '<div class = "col-6 mb-3" >
            <div class="card h-100" style="width: 100%;">
